...
early_stop_patience = 35 # so after 20 validations without improvement, stop training
...
data_split = [.6, .8] # 60% training, 20% validation, 20% test
# architecture='conv'
# input_dims=[28, 28, 1]
# output_dim=10
...
dataset = LoadDataset('MNIST', './data', train=True, transform=transform, subset_percentage=subset_percentage, seed=seed)
train_loader, val_loader, test_loader = dataset.get_data_loaders(batch_size=batch_size)
y_train = train_loader.dataset.dataset.targets
N = dataset.get_class_counts()
print("N:", N)
...
num_test = int(len(val_dataset) * subset_percentage)
num_test = int(len(test_dataset) * subset_percentage)
...
postnet_model = PosteriorNetwork(latent_dim, flow_models, N, num_classes, y_train, reg, dataset_name).to(device) 
...
############## TRAINING ######################
def train(model, optimiser, train_loader, val_loader, num_epochs, validation_every_steps, 
            ...
            if step % validation_every_steps == 0:
                ...
                with torch.no_grad():   
                    for batch_index, (X_val, y_val) in enumerate(val_loader):
                        ...
                        alpha = model(X_val, N_val) # gives a vector with alphas for each class
                ...
                val_accuracy = sum(val_correct) / val_length #The final batch of DataLoader may be smaller (drop_last=False).
...
train_losses, val_losses, train_accuracies, val_accuracies, all_train_losses = train(postnet_model, optimiser, train_loader, val_loader, 
