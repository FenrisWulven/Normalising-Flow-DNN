...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        #attention_out, attention = self.attention(x)
        # only want the tensor not the tuple
        attention_out, attention = self.attention(x)
...
class ViT(nn.Module):
    def __init__(self, image_size, channels, patch_size, embed_dim, num_heads, num_layers,
        ...
        self.to_patch_embedding = nn.Sequential(
            # We start with image shape: B x C x H x W 
            # and want to end up with B x num_patches x embed_dim (like in the Transformer from week 1)
        ...
        for i in range(num_layers):
            transformer_blocks.append(EncoderBlock(embed_dim=embed_dim, num_heads=num_heads, fc_dim=fc_dim, dropout=dropout))
    ...
    def forward(self, img):
        ...
        for block in self.transformer_blocks:
            x, weights = block(x)  # Collect attention weights from each block
